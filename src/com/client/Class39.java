...
final class Class39 {
	...
	private float method541(int i, int j, float f) {
		float f1 = anIntArrayArrayArray667[i][0][j]
				+ f
				* (anIntArrayArrayArray667[i][1][j] - anIntArrayArrayArray667[i][0][j]);
		f1 *= 0.001525879F;
	...
	private float method543(float f, int i, int j) {
		float f1 = anIntArrayArrayArray666[j][0][i]
				+ f
				* (anIntArrayArrayArray666[j][1][i] - anIntArrayArrayArray666[j][0][i]);
		f1 *= 0.0001220703F;
		return method542(f1);
	}
	...
	public int method544(int i, float f) {
		if (i == 0) {
			float f1 = anIntArray668[0] + (anIntArray668[1] - anIntArray668[0])
					* f;
			f1 *= 0.003051758F;
		...
		aFloatArrayArray669[i][0] = -2F * f2
				* (float) Math.cos(method543(f, 0, i));
		...
		for (int k = 1; k < anIntArray665[i]; k++) {
			...
			float f5 = f3 * f3;
			aFloatArrayArray669[i][k * 2 + 1] = aFloatArrayArray669[i][k * 2 - 1]
					* f5;
			aFloatArrayArray669[i][k * 2] = aFloatArrayArray669[i][k * 2 - 1]
					* f4 + aFloatArrayArray669[i][k * 2 - 2] * f5;
			for (int j1 = k * 2 - 1; j1 >= 2; j1--)
				aFloatArrayArray669[i][j1] += aFloatArrayArray669[i][j1 - 1]
						* f4 + aFloatArrayArray669[i][j1 - 2] * f5;
