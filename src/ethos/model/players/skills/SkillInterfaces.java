...
public class SkillInterfaces {
	 ...
	 * @param screen
	 * 
	 * @return Used to call all of the menus, redundant No change needed to the method no matter How many menus added
	 */
	 ...
	 * @param write []
	 * 
	 * @return Used to shorten the sidebar tab texts, shortens by up to 12x, Also includes the title of the menu and the current tab
	 */
	 ...
	 * @param lineCounter
	 * 
	 * @return Used to reduce code by 3x. Contains the item on interface, Level text, and the item description, along with the line Counter to ensure it is placed in the right spot
	 */
	...
	public void attackComplex(int screen) {
		if (screen == 1) {
			...
			menuLine(level, type + " Warhammer", 13576, 9);
			optionTab("Attack", type, "Bronze", "Iron", "Steel", "Black", "White", "Mithril", "Adamant", "Rune", "Dragon", "Barrows", "Special", "Milestones", "");
		} else if (screen == 10) {
			clearMenu();
			String level = "70";
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[c.playerAttack][0], 0);
	...
	public void strengthComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[c.playerStrength][0], 0);
	...
	public void defenceComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[c.playerDefence][0], 0);// Skill
																							// mastery
	...
	public void rangedComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[c.playerRanged][0], 1);
	...
	public void prayerComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[c.playerPrayer][0], 1);
	...
	public void hitpointsComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[c.playerHitpoints][0], 0);
	...
	public void agilityComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("30", "Varrock Rooftop Agility Course", 8007, 1);// Varrock
			menuLine("35", "Barbarian Outpost Agility Course", 1365, 2);// steel
			menuLine("52", "Wilderness Agility Course", 964, 3);// skull
			menuLine("60", "Seers Rooftop Agility Course", 6731, 4);// charos
			menuLine("90", "Ardougne Rooftop Agility Course", 8011, 5);// charos
	...
	public void herbloreComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("5", "Anti-poison \\n Marrentil potion (unf), Unicorn horn dust", 175, 1);
			menuLine("12", "Strength Potion \\n Tarromin potion (unf), Limpwurt root", 115, 2);
			menuLine("22", "Restore Potion \\n Harralander potion (unf), Red spider eggs", 127, 3);
			menuLine("26", "Energy Potion \\n Harralander potion (unf), Chocolate dust", 3010, 4);
			 menuLine("30", "Defence Potion \\n Ranarr potion (unf), White berries", 133, 5);
			menuLine("34", "Agility Potion \\n Toadflax potion (unf), Toad's legs", 3034, 6);
			menuLine("38", "Prayer Restore Potion \\n Ranarr potion (unf), Snape grass", 139, 7);
			menuLine("45", "Super Attack Potion \\n Irit potion (unf), Eye of newt", 145, 8);
			menuLine("48", "Super Anti-poison \\n Irit potion (unf), Unicorn horn dust", 181, 9);
			menuLine("50", "Fishing Potion \\n Avantoe potion (unf), Snape grass", 151, 10);
			menuLine("52", "Super Energy Potion \\n Avantoe potion (unf), Mort myre fungus", 3018, 11);
			menuLine("55", "Super Strength Potion \\n Kwuarm potion (unf), Limpwurt root", 157, 12);
			menuLine("60", "Weapon Poison \\n Kwuarm potion (unf), Dragonscale dust", 187, 13);
			menuLine("63", "Super Restore potion \\n Snapdragon potion (unf), Red spider eggs", 3026, 14);
			menuLine("66", "Super Defence potion \\n Cadantine potion (unf), White berries", 163, 15);
			menuLine("68", "Antidote+ \\n Toadflax potion (unf), Yew root", 5945, 16);
			menuLine("69", "Antifire potion \\n Lantadyme potion (unf), Dragonscale dust", 2454, 17);
			menuLine("72", "Ranging potion \\n Dwarf weed potion (unf), Wine of zamorak", 169, 18);
			menuLine("73", "Weapon poison+ \\n Coconut milk, Cactus spine, Red spiders egg ", 5937, 19);
			menuLine("76", "Magic potion \\n Lantadyme potion (unf), Potato cactus", 3042, 20);
			menuLine("77", "Stamina potion \\n Super energy(4), 3 x Amylase crystal", 12625, 21);
			menuLine("78", "Zamorak brew \\n Torstol potion (unf), Jangerberries", 189, 22);
			menuLine("79", "Antidote++ \\n Irit potion (unf), Magic root", 5954, 23);
			menuLine("81", "Saradomin brew \\n Toadflax potion (unf), Birds nest", 6687, 24);
			menuLine("82", "Weapon poison++ \\n Coconut milk, Cave nightshade, Poison ivy berries", 5940, 25);
			menuLine("90", "Super Combat \\n Torstol potion (unf), 1 x super attack, strength\\nand defence potion", 12695, 26);
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[c.playerHerblore][0], 0);
	...
	public void thievingComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill mastery", c.getItems().skillcapes[c.playerThieving][0], 0);
	...
	public void fletchingComplex(int screen) {
		if (screen == 1) {
			...
			optionTab("Fletching", "Darts", "Arrows", "Bows", "Darts", "Javelins", "Milestones", "", "", "", "", "", "", "", "");
		}
			...
			clearMenu();
			menuLine("1", "Bronze Javelin", 825, 0);
			menuLine("17", "Iron Javelin", 826, 1);
			menuLine("32", "Steel Javelin", 827, 2);
			menuLine("47", "Mithril Javelin", 828, 3);
			menuLine("62", "Adamant Javelin", 829, 4);
			menuLine("77", "Rune Javelin", 830, 5);
			menuLine("92", "Dragon Javelin", 19484, 6);
			optionTab("Fletching", "Darts", "Arrows", "Bows", "Darts", "Javelins", "Milestones", "", "", "", "", "", "", "", "");
		}
	...
	public void slayerComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[c.playerSlayer][0], 0);
	...
	public void miningComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("99", "Sill Mastery", 14115, 1);
	...
	public void fishingComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[c.playerFishing][0], 1);
	...
	public void cookingComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[c.playerCooking][0], 1);
	...
	public void firemakingComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[c.playerFiremaking][0], 0);
	...
	public void farmingComplex(int screen) {
		if (screen == 1) {
			...
			clearMenu();
			menuLine("99", "Skill Mastery", c.getItems().skillcapes[c.playerFarming][0], 0);
	...
	public void hunterComplex(int screen) {
		if (screen == 1) {
			...
			menuLine("53", "Chinchompas (woodland)", 9976, 1);
			menuLine("63", "Red chinchompas (jungle)", 9977, 2);
			menuLine("73", "Black chinchompas (wilderness lv32)", 11959, 3);
